...
?>
	...
	<p class="short_desc">Traitement de l'ajout d'un logiciel</p>

	<h3 class="titre">Voici un log du d&eacute;roulement de l'ajout :</h3><hr />
	...
	<p id="log">
		...
		else
		{
			...
			// On verifie que le logiciel existe pas deja...
			$requete = mysql_query("SELECT nom FROM fiche_soft WHERE id='$id'");
			$test_valide = mysql_fetch_array($requete);
			
		
			if ($test_valide['0'] != NULL) {
				$erreur .= 'Erreur : Cette fiche existe d&eacute;j&agrave;. Veuillez choisir un id diff&eacute;rent.<br />';
			}

			echo 'V&eacute;rification de l\'existance de la fiche.<br />';
			...
			if ($erreur == NULL) {
				...
				//Partie SQL
				$requete = "INSERT INTO fiche_soft VALUES ('','$id','$nom','','$telechargement','$version','$taille','$os','$licence','$date','$langue','$screen','$description','$short_desc','$site','','','','$categorie', '$id_framasoft', '1')";
				...
				$requete = "INSERT INTO logs VALUES ('','new','$id')";
				mysql_query($requete) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
				...
				if ($screen != NULL AND $nb_screen == NULL) {
					$nb_screen = 1;
				}
