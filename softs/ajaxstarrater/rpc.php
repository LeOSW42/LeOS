...
$query = mysql_query("SELECT total_votes, total_value, used_ips FROM ".$prefixe."$rating_tableName WHERE id='$id_sent' ")or die(" Error: ".mysql_error());
...
$voted=mysql_num_rows(mysql_query("SELECT used_ips FROM ".$prefixe."$rating_tableName WHERE used_ips LIKE '%".$ip."%' AND id='".$id_sent."' "));
if(!$voted) {     //if the user hasn't yet voted, then vote normally...

	if (($vote_sent >= 1 && $vote_sent <= $units) && ($ip == $ip_num)) { // keep votes within range, make sure IP matches - no monkey business!
		$update = "UPDATE ".$prefixe."$rating_tableName SET total_votes='".$added."', total_value='".$sum."', used_ips='".$insertip."' WHERE id='$id_sent'";
		$result = mysql_query($update);		
	} 
} //end for the "if(!$voted)"
// these are new queries to get the new values!
$newtotals = mysql_query("SELECT total_votes, total_value, used_ips FROM ".$prefixe."$rating_tableName WHERE id='$id_sent' ")or die(" Error: ".mysql_error());
